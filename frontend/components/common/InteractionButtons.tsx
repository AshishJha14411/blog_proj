...
export default function InteractionButtons({
  ...
  return (
    <div className="flex items-center gap-4">
      <button onClick={handleLike} disabled={!isAuthenticated} className={`p-2 rounded-full transition-colors ${!isAuthenticated ? 'cursor-not-allowed' : ''}`}>
        {/* SVG for Heart Icon */}
      ...
      <button onClick={handleBookmark} disabled={!isAuthenticated} className={`p-2 rounded-full transition-colors ${!isAuthenticated ? 'cursor-not-allowed' : ''}`}>
