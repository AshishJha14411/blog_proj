...
export async function sendFeedback(postId: number, feedback: string): Promise<PostOut> {
  const { data } = await api.post<PostOut>(`/stories/${postId}/feedback`, { feedback });
  return data;
}

export async function publishStory(postId: number): Promise<PostOut> {
  const { data } = await api.post<PostOut>(`/stories/${postId}/publish`, {});
  return data;
}

export async function unpublishStory(postId: number): Promise<PostOut> {
